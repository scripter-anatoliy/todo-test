...
import { FILTER_ALL, FILTER_ACTIVE, FILTER_COMPLETED, FilterType } from "../../../types";
...
describe("FilterButtons", () => {
  ...
  test("записываем правильные значения при клике", async () => {
    const { allButton, activeButton, completedButton, mockOnSetFilter } = setup();
  ...
  test("стиль кнопки нажатой изменился", () => {
    const { rerender } = render(
      <FilterButtons filter={FILTER_ALL} onSetFilter={jest.fn()} />
    );
    
    expect(screen.getByRole("button", { name: /all/i })).toHaveClass("selected");
    expect(screen.getByRole("button", { name: /active/i })).not.toHaveClass("selected");

    rerender(<FilterButtons filter={FILTER_ACTIVE} onSetFilter={jest.fn()} />);
    expect(screen.getByRole("button", { name: /active/i })).toHaveClass("selected");
  });
});
